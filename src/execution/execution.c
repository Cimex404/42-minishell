...
/*   Updated: 2025/02/10 08:54:01 by jgraf            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

/*	These functions are used to execute the input.
*	For each pipe and redirection, a child process is created.
*	When the child process is finished, the output is sent to the input of the next command.
*	When all processes are finished, the parent process prints the final output.
*/
...
static void	execute_command(t_process_args args, t_file *files)
{
	...
	if (args.cmd_end < args.tokens->token_count && args.tokens->tokens[args.cmd_end].type == PIPE)
		setup_pipe(args.data->pipe_fds);
	pid = fork();
	if (pid == -1)
		handle_error(FORK_ERROR, NULL, args.tokens);
	else if (pid == 0)
		handle_child(args.data, (args.cmd_end == args.tokens->token_count), files, args.tokens->env);
	else
	{
		handle_parent(args.data, &args.data->prev_fd, pid, (args.cmd_end == args.tokens->token_count));
...
static void	process_command(t_process_args *args)
{
	...
	{
		args->data->full_path = resolve_command_path(args->tokens->env, args->data->cmd[0]);
		if (!args->data->full_path && !is_builtin(args->data->cmd[0]))
			handle_error(COMMAND_NOT_FOUND, args->data->cmd[0], args->tokens);
		else if (access(args->data->full_path, X_OK) != 0 && !is_builtin(args->data->cmd[0]))
