...
/*   Updated: 2025/02/10 08:54:59 by jgraf            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

//	These functions are used to handle the redirections.
...
void	get_redir(t_input tok, int *cmd_start, int *cmd_end, t_file *files)
{
	...
	while (i < *cmd_end)
	{
		...
		{
			if (i + 1 >= *cmd_end)
				return (handle_error(INVALID_INPUT, tok.tokens[i].token, &tok), (void)0);
			redir_type(&tok, tok.tokens[i], tok.tokens[i + 1], files);
			if (tok.tokens[i].type == HEREDOC && !files->infile)
				return (handle_error(INVALID_INPUT, tok.tokens[i + 1].token, &tok), (void)0);
