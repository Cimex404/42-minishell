...
/*   Updated: 2025/02/10 08:52:50 by jgraf            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

/*	These functions are used to split the tokens into commands.
*	Commands are split by redirections, pipes and the end of the input.
*/
...
char	**parse_cmd(t_input tokens, int cmd_start, int cmd_end)
{
	...
	if (!cmd)
		return (NULL);
	result = parse_part_2((t_parse_args){cmd_start, cmd_end, tokens, cmd, valid_tokens});
	if (!result)
	{
		free_cmd_array(cmd);
		return (NULL);
