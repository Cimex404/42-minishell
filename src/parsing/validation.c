...
/*   Updated: 2025/02/10 08:55:54 by jgraf            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

/*	These functions are used to validate the input.
*	In case of unclosed quotes, pipes or missing arguments, the program will
*	exit with the correct error code and give the prompt back.
*/
...
int	validate_input(t_input *tokens)
{
	if (!tokens || !tokens->tokens || tokens->token_count == 0
		|| tokens->quote_error == true || check_valid_pipes(tokens) > 0)
		return (1);
