...
/*   Updated: 2025/02/10 08:56:50 by jgraf            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

//	These functions are used to determine the type of a token and to handle special characters.
...
char	*handle_specials(const char *input, int start, int *i)
{
	...
	if (is_special_char(input[start]) && input[start] != ' ' && input[start] != '\t')
	{
